...
def fetch_wdm_yields() -> tuple[list[dict], date]:
    ...
    for row in reader:
        if trade_date is None:
            trade_date = datetime.strptime(row["Trade date"].strip(), "%d-%b-%Y").date()
    ...
    if trade_date is None:
        raise RuntimeError("No data rows in CSV")
...
if __name__ == "__main__":
    print("Fetching WDM yields...")
    try:
        observations, trade_date = fetch_wdm_yields()
