...
$$\boldsymbol{\beta}_{\text{base}} = (\beta_0, \beta_1, \beta_2, \beta_3, \tau_1, \tau_2)$$

The user applies a set of shocks resulting in shocked parameters:

$$\boldsymbol{\beta}_{\text{shocked}} = (\beta_{0, \text{shocked}}, \beta_{1, \text{shocked}}, \beta_{2, \text{shocked}}, \beta_{3, \text{shocked}}, \tau_1, \tau_2)$$
...
We define a first-order / linear P&L attribution for each parameter $\beta_i$ ($i \in \{0, 1, 2, 3\}$). We compute the sensitivity of the bond price $P$ to each parameter $\beta_i$ using a symmetric central finite difference at the base curve:
...
Using the independent key rate duration (KRD) engine, each bond position has key rate durations $KRD_k$ at the benchmark tenors:

$$t_k \in \{0.25, 0.5, 1.0, 2.0, 3.0, 5.0, 7.0, 10.0, 15.0, 20.0, 30.0, 40.0\}$$
...
At the portfolio level, the sum of portfolio key rate sensitivities (the sum over all positions of their respective $\text{KRS}_k$) reconciles with the total portfolio parallel DV01. This provides a complete breakdown of parallel yield risk across the tenor grid.
